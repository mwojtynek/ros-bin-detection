...
#		roslaunchProcess = subprocess.Popen(["roslaunch", "perception", "perception.launch"])
#		time.sleep(2.0)		
...
class move_to_init(smach.StateMachine):
	...
	def execute(self, userdata):
		...
		# set Planner
		mgc.set_planner_id('PRMkConfigDefault')
...
#		Q1 = [0.0,-0.785,0.785,-3.14,-1.57,0] 	# cam init 1
		Q1 = [-3.14,-2.356,-1.57,-0.785,1.57,0]	# init: Base = -173 Grad auf Bedienpanel
...
#		rospy.sleep(2.0)
		...
		# set Planner
		ikf.set_planner_id('PRMkConfigDefault')
    ...
    # Get the current pose so we can add it as a waypoint
		...
		target_pose.pose.position.z = round(get_coordinates.z, 2) + 0.7
